        ...
        switch (op[i].opcode) {
            ...
            case VARY:
                if(!frames_set){
                    ...
                    strcpy(name, "frame");
                    printf("No basename provided, using default basename '%s'\n", name);
                }
                break;
        }
    }
...
struct vary_node ** second_pass() {
    ...
    double dval, frame_start, frame_end, val_start, val_end;

    for(int i = 0; i < lastop; i++){
        switch(op[i].opcode){
            case VARY:
                frame_start = op[i].op.vary.start_frame;
                frame_end = op[i].op.vary.end_frame;
                val_start = op[i].op.vary.start_val;
                val_end = op[i].op.vary.end_val;

                dval = (val_end - val_start) / (frame_end - frame_start);
                ...
                for(int f = frame_start; f <= frame_end; f++){
                    ...
                    new->value = val_start + (f - frame_start) * dval;
                    new->next = NULL;
